...
// 臉部特徵點索引 (MediaPipe Face Mesh 標準索引)
  ...
  if (video && video.elt.srcObject) {
    if (isHost && video.elt.videoWidth > 0) {
      ...
      for (let face of faces) {
        ...
        stroke(57, 255, 20); 
        beginShape();
        for (let i of faceConnections.outline) {
          let pt = face.keypoints[i];
          vertex(pt.x * s + xOff, pt.y * s + yOff);
        }
        endShape();
        ...
        stroke(0, 255, 255);
        beginShape();
        for (let i of faceConnections.leftEye) {
          let pt = face.keypoints[i];
          vertex(pt.x * s + xOff, pt.y * s + yOff);
        }
        endShape();
        beginShape();
        for (let i of faceConnections.rightEye) {
          let pt = face.keypoints[i];
          vertex(pt.x * s + xOff, pt.y * s + yOff);
        }
        endShape(CLOSE);
        ...
        stroke(255, 0, 255);
        beginShape();
        for (let i of faceConnections.mouth) {
          let pt = face.keypoints[i];
          vertex(pt.x * s + xOff, pt.y * s + yOff);
        }
        endShape(CLOSE);
